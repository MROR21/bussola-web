...
export function NivelamentoPage({
  ...
  async function concluir(perfil: Perfil, squadId: string) {
    setErro(null)
    try {
      await salvarPerfil(usuario.id, perfil, squadId)
      atualizarUsuario({ squadId }) // mantém o squad da sessão em dia (o filtro de Fluxos usa ele)
      onConcluir(perfil)
    } catch (e) {
  ...
  return (
    <main className="relative flex min-h-screen flex-col items-center justify-center gap-6 overflow-hidden bg-navy-900 px-4 py-12 text-neutral-100">
      <CompassRose
        className="pointer-events-none absolute left-1/2 top-1/2 size-[900px] -translate-x-1/2 -translate-y-1/2 text-gold-500 opacity-[var(--opacidade-marca-dagua)]"
      />
